...
const app = readFileSync("src/app/YouTraderApp.tsx", "utf8");
const identity = readFileSync("src/billing/revenueCatIdentity.ts", "utf8");
assert.match(app, /RevenueCatIdentitySynchronizer/, "the client uses RevenueCatIdentitySynchronizer");
assert.match(app, /logIn:\s*\(appUserID\)\s*=>\s*Purchases\.logIn\(appUserID\)/, "the client identifies RevenueCat with the Supabase UUID via Purchases.logIn");
assert.match(identity, /normalizeUserId/, "identity helper rejects non-UUID app user ids");
assert.doesNotMatch(app, /Purchases\.logIn\(\s*session\.user\.email/);
