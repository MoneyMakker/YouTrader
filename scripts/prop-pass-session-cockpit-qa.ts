...
for (const required of ["Check Next Trade", "Today’s Plan", "Contract Calculator", "What-If Simulator", "Risk Rules", "Payout Planner", "Safe Withdrawal", "Timeline", "Decision Replay", "Capital Preservation", "Survival Capacity", "Daily Risk Calendar", "Breach Replay", "Session Lock"]) assert.match(cockpit, new RegExp(required.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), `Cockpit route missing: ${required}`);
