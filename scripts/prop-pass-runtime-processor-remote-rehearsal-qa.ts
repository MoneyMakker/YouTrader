...
async function createDisposableUser(label: string) {
  const email = `yt-b117-reh-${label}-${randomUUID().slice(0, 8)}@youtrader.qa`;
  const password = `Qa!${randomUUID().replace(/-/g, "").slice(0, 20)}`;
  const created = await admin().auth.admin.createUser({
    email,
    password,
    email_confirm: true,
    user_metadata: { yt_disposable: true, yt_suite: "build117-processor-rehearsal" },
  });
...
async function scrubUser(userId: string) {
  ...
  await admin().auth.admin.updateUserById(userId, {
    ...
    user_metadata: {
      yt_disposable: false,
      yt_suite: "build117-processor-rehearsal-scrubbed",
  ...
  return {
    ...
    scrubbed:
      ...
      meta.yt_suite === "build117-processor-rehearsal-scrubbed",
  };
}
...
async function scrubOrphans() {
  ...
  for (const user of listed.data.users ?? []) {
    ...
    if (!(email.startsWith("yt-b117-reh-") || meta.yt_suite === "build117-processor-rehearsal")) continue;
  ...
  for (const userId of createdUserIds) {
    try {
      ...
      if (listed.data.user) {
        ...
        if (email.startsWith("yt-b117-reh-") || meta.yt_suite === "build117-processor-rehearsal") activeDisposable += 1;
      }
    } catch {
      cleanupPass = false;
    }
...
const report = {
  suite: "prop-pass-runtime-processor-remote-rehearsal",
...
writeFileSync(join(outDir, `RUNTIME_PROCESSOR_REMOTE_REHEARSAL_${stamp}.json`), JSON.stringify(report, null, 2));
writeFileSync(join(outDir, "RUNTIME_PROCESSOR_REMOTE_REHEARSAL_LATEST.json"), JSON.stringify(report, null, 2));
...
console.log("prop-pass-runtime-processor-remote-rehearsal: PASS");
