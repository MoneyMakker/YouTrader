...
async function createDisposableUser(label: string) {
  const email = `yt-b117-prod-rt-${label}-${randomUUID().slice(0, 8)}@youtrader.qa`;
  const password = `Qa!${randomUUID().replace(/-/g, "").slice(0, 20)}`;
  const created = await admin().auth.admin.createUser({
    email,
    password,
    email_confirm: true,
    user_metadata: { yt_disposable: true, yt_suite: "build117-runtime-production" },
  });
...
async function scrubUser(userId: string) {
  ...
  await admin().auth.admin.updateUserById(userId, {
    ...
    user_metadata: {
      yt_disposable: false,
      yt_suite: "build117-runtime-production-scrubbed",
  ...
  return {
    deletedAuth: false,
    scrubbed: Boolean(bannedUntil) || email.includes("@youtrader.qa.invalid") || meta.yt_suite === "build117-runtime-production-scrubbed",
  };
}
...
async function scrubOrphans() {
  ...
  for (const user of listed.data.users ?? []) {
    ...
    if (!(email.startsWith("yt-b117-prod-rt-") || meta.yt_suite === "build117-runtime-production")) continue;
  ...
  for (const userId of createdUserIds) {
    try {
      ...
      if (listed.data.user) {
        ...
        if (email.startsWith("yt-b117-prod-rt-") || meta.yt_suite === "build117-runtime-production") activeDisposable += 1;
      }
    } catch {
      cleanupPass = false;
    }
...
const report = {
  suite: "prop-pass-runtime-state-production",
...
writeFileSync(join(outDir, `RUNTIME_STATE_PRODUCTION_${stamp}.json`), JSON.stringify(report, null, 2));
writeFileSync(join(outDir, "RUNTIME_STATE_PRODUCTION_LATEST.json"), JSON.stringify(report, null, 2));
...
console.log("prop-pass-runtime-state-production: PASS");
