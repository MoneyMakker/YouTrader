#!/usr/bin/env bash
# Phase 2A remediation — live authenticated vertical slice. Does NOT touch production.
set -euo pipefail
...
DB=prop_os_live2a
export PROP_OS_LIVE_SLICE_DB="$DB"

if ! pg_isready -h "$PGHOST" -p "$PGPORT" >/dev/null 2>&1; then
  echo "prop-pass-live-vertical-slice-qa: SKIP — local postgres :55432 not ready"
  exit 0
fi

if [[ ! -f .tmp/bootstrap_supabase_roles.sql ]]; then
  echo "prop-pass-live-vertical-slice-qa: SKIP — missing .tmp/bootstrap_supabase_roles.sql"
  exit 0
fi
