#!/usr/bin/env bash
# Phase 1C — isolated PostgreSQL shadow QA (smoke + full round-trip remediation).
...
echo "prop-os-shadow-pg-qa: migrations applied; running round-trip QA"
node --import ./scripts/prop-os-register.mjs --experimental-strip-types --experimental-transform-types \
  scripts/prop-os-shadow-pg-roundtrip-qa.ts
