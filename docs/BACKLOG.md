...
22. Final persistence schema and RLS — IN PROGRESS; minimal 13-file SQL set + clean local prod-baseline rehearsal documented in `PPOS-117_FINAL_SQL_MIGRATION_APPROVAL.md`; processor packet separate in `PPOS-117_RUNTIME_PROCESSOR_DEPLOYMENT_APPROVAL.md`; PI migrations excluded (REQUIRED_ONLY_IF_PI_ENABLED); production apply still pending explicit PO SQL approval.
