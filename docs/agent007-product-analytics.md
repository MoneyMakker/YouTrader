...
Events are buffered in AsyncStorage in batches of at most 25. Delivery is delayed until the app is active, retries are bounded to three attempts, and a stable idempotency key prevents replay from creating extra records. A queue is cleared on sign-out to avoid cross-account delivery on a shared device.
