...
export function compareRiskModes(rooms: RiskRooms, context: "challenge" | "live", lossPerContractMinor: MoneyMinor | null): ModeComparison[] {
  return (["calm", "balanced", "gambler"] as const).map((mode) => {
    ...
    const recommendedContracts = enabled && hasVerifiedContractRisk ? Math.floor(risk / lossPerContractMinor) : 0;
    return { mode, highRisk: mode === "gambler", riskPerTradeMinor: enabled ? risk : 0, maximumDailyRiskMinor: daily, maximumTrades: enabled ? trades : 0, stopAfterLosses: mode === "gambler" ? 1 : 2, recommendedContracts, requiresInstrumentSetup: !hasVerifiedContractRisk, projectedRemainingBufferMinor: daily == null ? null : Math.max(0, daily - damage), maximumProjectedDamageMinor: enabled ? damage : 0, enabled, requiresConfirmation: mode === "gambler" && enabled };
  });
}
