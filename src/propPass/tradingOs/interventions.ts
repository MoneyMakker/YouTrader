...
import { formatMoneyMinor, moneyClampNonNegative, moneyMultiplyInteger, moneySubtract } from "./financialMath";
...
export function assessSmartInterventions(input: SmartInterventionInput): SmartIntervention[] {
  ...
  if (input.proposed.riskMinor > input.plan.riskPerTradeMinor) warn("buffer_consumption", "Risk exceeds today’s plan", "The proposed risk is above the frozen per-trade plan.", `Proposed risk is $${formatMoneyMinor(moneySubtract(input.proposed.riskMinor, input.plan.riskPerTradeMinor))} above plan.`, "Use the recommended size.");
