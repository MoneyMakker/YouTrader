...
export function ValueOnboarding({ phase, onPhaseChange, onComplete, onExistingAuth }: Props) {
  ...
  const personalization = phase === "onboarding_personalization";
  ...
  const goTo = (next: number) => {
    ...
    Animated.sequence([
      Animated.timing(transitionX, { toValue: next > current ? -18 : 18, duration: 120, useNativeDriver: true }),
  ...
  const panResponder = useRef(PanResponder.create({
    ...
    onPanResponderRelease: (_, gesture) => {
      if (gesture.dx < -45) goTo(indexRef.current + 1);
      else if (gesture.dx > 45) goTo(indexRef.current - 1);
    },
  })).current;
