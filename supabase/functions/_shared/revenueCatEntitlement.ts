...
function isActiveSubscription(row: SubscriptionRow | null) {
  ...
  // Align with client: billing retry window still grants Pro until expires_at.
...
async function fetchRevenueCatEntitlement(userId: string, entitlementId: string) {
  ...
  if (!secret) {
    console.warn("[YouTrader:subscription] revenuecat_secret_missing", { user_id: userId, entitlement_id: entitlementId });
    return null;
  }
...
export async function resolveServerProEntitlement(
  ...
  // Must match RevenueCat entitlement lookup_key and EXPO_PUBLIC_REVENUECAT_ENTITLEMENT_ID.
  const entitlementId = env("REVENUECAT_ENTITLEMENT_ID") || "YouTrader Pro";
  ...
  console.log("[YouTrader:subscription] entitlement_check", {
    user_id: userId,
    entitlement_id: entitlementId,
    subscription_found: subscriptionFound,
    status: row?.status || null,
    has_expires_at: !!row?.expires_at,
  });
  ...
  console.log("[YouTrader:subscription] revenuecat_entitlement_checked", {
    user_id: userId,
    entitlement_id: entitlementId,
    validated: valid,
    has_expires_at: !!revenueCatEntitlement?.expires_date,
    has_product_id: !!revenueCatEntitlement?.product_identifier,
  });
  ...
  console.log("[YouTrader:subscription] entitlement_synced", {
    user_id: userId,
    entitlement_id: entitlementId,
    subscription_found: subscriptionFound,
  });
